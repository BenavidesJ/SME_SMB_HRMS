...
export const crearSolicitudHoraExtra = async ({
  ...
  try {
    ...
    if (!Number.isFinite(idColab)) throw new Error("id_colaborador debe ser numérico");

    const fechaTxt = String(fecha_trabajo ?? "").trim();
    if (!fechaTxt) throw new Error("fecha_trabajo es obligatoria (YYYY-MM-DD)");
    ...
    if (!Number.isFinite(idTipoHx)) throw new Error("id_tipo_hx debe ser numérico");

    const justi = String(justificacion ?? "").trim();
    if (!justi) throw new Error("justificacion es obligatoria");
    if (justi.length > 100) throw new Error("justificacion no puede exceder 100 caracteres");

    const tipoHx = await TipoHoraExtra.findByPk(idTipoHx, {
      attributes: ["id_tipo_hx", "nombre", "multiplicador"],
      transaction: tx,
    });
    if (!tipoHx) throw new Error(`No existe tipo de hora extra con id ${idTipoHx}`);
    ...
    const duplicada = await SolicitudHoraExtra.findOne({
      where: { id_colaborador: idColab, fecha_trabajo: fechaTxt, estado: ESTADO_PENDIENTE_ID },
