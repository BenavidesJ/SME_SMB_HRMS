...
export const crearIncapacidad = async ({
  ...
  try {
    ...
    if (!Number.isFinite(Number(id_tipo_incap))) {
      throw new Error("id_tipo_incap es requerido y debe ser numérico");
    }

    const tipo = await TipoIncapacidad.findByPk(Number(id_tipo_incap), {
    ...
    const row = await Incapacidad.create(
      {
        ...
        id_tipo_incap: Number(id_tipo_incap),
        fecha_inicio,
        fecha_fin,
        porcentaje_patrono,
        porcentaje_ccss,
