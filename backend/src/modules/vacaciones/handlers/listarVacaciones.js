...
export async function listarVacaciones({ id_colaborador, estado } = {}) {
  let items = [];

  if (id_colaborador !== undefined && id_colaborador !== null && String(id_colaborador).trim() !== "") {
    items = await listarVacacionesPorColaborador({ id_colaborador });
  } else {
    ...
    for (const row of rows) {
      ...
      const collaboratorItems = await listarVacacionesPorColaborador({ id_colaborador: collaboratorId });
