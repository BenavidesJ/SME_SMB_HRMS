...
import { DATE_FMT, formatDateRangeUi, getWeekRangeSundayToSaturday, now } from "../utils/dates";
...
export function useWeekPager(initial?: Dayjs) {
  ...
  const label = useMemo(
    () => formatDateRangeUi(range.start.format(DATE_FMT), range.end.format(DATE_FMT)),
    [range],
  );
